...
class Blockchain:
    ...
    def register_difficulty(self, difficulty):
        """
        Add a new node to the list of nodes
        """
        #Checking node_url has valid format
    ...
    def valid_proof(self, transactions, last_hash, nonce, difficulty=MINING_DIFFICULTY):
        ...
        """
        difficulty= int(self.difficulty[0])
...
def register_nodes():
    values = request.form
    if not values.get('difficulty') or int(values.get('difficulty')) < 1 :
        return "Difficulty must be greater than 0"
    
    difficulty = values.get('difficulty').replace(" ", "").split(',')
    blockchain.register_difficulty(difficulty)
    if values.get('nodes'):
...
def get_nodes():
    ...
    response = {'nodes': nodes,'difficulty':difficulty}
    return jsonify(response), 200
